...
%homepage\footnote{\url{http://www.cs.uni-duesseldorf.de/AG/BI/Software/SyBiL/}\label{sfoot}}.
...
\begin{inparaenum}[i)]
\item in tabular form, or
\item in an XML based format (SBML).
\end{inparaenum}
...
name of the reaction list, which must be there.
\begin{description}
...
appended to each metabolite name (e.\,g. for transport reactions):
\begin{verbatim}
h2o[e] <==> h2o[c]
\end{verbatim}
...
In order to read SBML written model files, the package \pkg{sybilSBML} is
required (which is also available from available from CRAN\footref{cranfoot}).
...
argument quote (see also documentation of \Comp{read.table}).
...
Settings to \pkgname{} can be set using the function \Comp{SYBIL\_SETTINGS}.
Set parameter solver to \cplex for every optimization.
<<eval=FALSE>>=
SYBIL_SETTINGS("SOLVER", "cplex")
@
Now, \cplex is used as default solver e.\,g. in \Comp{simpleFBA}.
...
or SBML-Files (with package \pkg{sybilSBML}). See section \ref{inputformats} on
