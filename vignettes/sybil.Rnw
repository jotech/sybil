...
CRAN\footnote{\url{http://cran.r-project.org/}\label{cranfoot}}, or, from
...
tab-delimited, the default is \Comp{.tsv}. Function \Comp{readTSVmod()} returns
an object of class \Comp{modelorg}:
<<>>=
mod
@
...
<<>>=
optimizeProb(Ec_core, algorithm = "fba", retOptSol = FALSE)
@
...
solution. At first, an objective value calculated via FBA is required:
<<>>=
...
length(fl)
@
The flux distribution for the exchange reactions can be retrieved in a similar
way.
<<>>=
ex <- findExchReact(Ec_core)
getFluxDist(mtf, ex)
@
Function \Comp{findExchReact()} returns all exchange reactions present in a
metabolic network (section~\ref{envir}).  The value for the objective function
given in the model (here: biomass production) can be accessed by method
\Comp{mod\_obj}:
<<>>=
mod_obj(mtf)
@
...
extending class \Comp{optsol\_optimizeProb} (see section~\ref{fba}).
Method \Comp{checkOptSol} gives an overview about the results and status of
the optimizations. Additionally, class \Comp{optsol} contains a
method \Comp{histogram}, plotting the values of the objective function
...
the deletion of gene $i$ results in a maximum growth ratio of zero.
<<>>=
...
costs after the optimization, use argument \Comp{poCmd} of method
\Comp{optimizeProb} of class \Comp{sysBiolAlg} (see also
section~\ref{classsba}).
<<>>=
opt <- optimizeProb(Ec_core, MoreArgs = list(poCmd = list("getRedCosts")))
postProc(opt)
@
